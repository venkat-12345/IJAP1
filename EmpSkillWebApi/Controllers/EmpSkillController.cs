...
namespace EmpSkillWebApi.Controllers
{
    public class EmpSkillController : ApiController
    {
        ...
        {
            List<EmpSkill> empskills = await emprepo.GetAllEmployeesAsync();
            return Ok(empskills);
        }
        ...
        {
            List<EmpSkill> empIds = await emprepo.GetBySkillIdAndEmpIdAsync(eid, sid);
            return Ok(empIds);
        }
        ...
        public async Task<IHttpActionResult> GetByEmpId(string EmpId)
        {
            try
            {
                List<EmpSkill> empskills = await emprepo.GetByEmpIdAsync(EmpId);
                return Ok(empskills);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IHttpActionResult> Insert(EmpSkill empskl)
        {
            ...
            return Created<EmpSkill>("api/Flight", empskl);
        }
